pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "lington/currencyservice"
    ...
    stages {
        ...
        stage('Manual Approval for Main Branch Update') {
            steps {
                script {
                    slackSend(channel: SLACK_CHANNEL, message: " *Manual Approval Required for currencyservice pipeline!* Please approve deployment to the *main* branch.")
                }
    ...
    post {
        success {
            script {
                slackSend(channel: SLACK_CHANNEL, message: "✅ *Build Succeeded!* Jenkins has successfully completed the currencyservice pipeline. 🎉")
            }
        }
        failure {
            script {
                slackSend(channel: SLACK_CHANNEL, message: "❌ *Build Failed! for currencyservice pipeline* Please check the Jenkins logs for details. 🔴")
            }
        }
    }
}
