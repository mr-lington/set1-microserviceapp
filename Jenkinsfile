pipeline {
    ...
    environment {
        ...
        IMAGE_NAME = "franklinonyia/emailservice"
    ...
    stages {
        ...
        stage('Manual Approval for Main Branch Update') {
            steps {
                script {
                    slackSend(channel: SLACK_CHANNEL, message: " *Manual Approval Required for emailservice pipeline!* Please approve deployment to the *main* branch.")
                }
    ...
    post {
        success {
            script {
                slackSend(channel: SLACK_CHANNEL, message: "✅ *Build Succeeded!* Jenkins has successfully completed the emailservice pipeline. 🎉")
            }
        }
        failure {
            script {
                slackSend(channel: SLACK_CHANNEL, message: "❌ *Build Failed! for emailservice pipeline* Please check the Jenkins logs for details. 🔴")
            }
        }
    }
}
